...
def test_loom_design_version_2_2_0_consistent() -> None:
    ...
    assert claude_manifest["version"] == "2.4.2"
    assert codex_manifest["version"] == "2.4.2"
    assert agy_manifest["version"] == "2.4.2"
    assert "## [2.4.2]" in changelog
    assert "## [2.2.1]" not in changelog
    readme_pins = {
        "README.md": "| [`loom-design`](loom-design/) | 2.4.2 |",
        "loom-design/README.md": "**Version**: 2.4.2",
        "loom-design/README.ja.md": "**Version**: 2.4.2",
        "loom-design/README.zh-TW.md": "**Version**: 2.4.2",
    }
    for name, pin in readme_pins.items():
        assert pin in (REPO / name).read_text(encoding="utf-8"), name
    assert "Version 2.4.2. Turns a rough idea" in (REPO / "README.md").read_text(encoding="utf-8")
