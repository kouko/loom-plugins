...
2. Quality comes from machines, not from the user's sign-off: a mechanically computed reviewer floor — one reviewer only when the checker proves the whole change is narrow and low-risk, and two reviewers for every other or undecidable change — plus independent acceptance testing by someone who did not write it (omitted only when every Acceptance line is mechanical), and an adversarial pass; every incident becomes a permanent eval. A step falls outside this guarantee in two cases, and only these two: the user skips it by plain instruction, and the checker proves from the branch's committed delta that the change is narrow, which is stricter than the computation that sets the reviewer floor: the delta must also carry no file anything executes and remove no test, in which case the spec, plan, acceptance-test and adversarial steps are skipped with no one asked. Every skip of either kind must be disclosed on the pull request, and the attestation-level verification status there is recomputed rather than claimed.
...
Work that passes every gate while its quality is insufficient and the user cannot see that it is — the user says "OK" to an acceptance test report that hides the problem.
