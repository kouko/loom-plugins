...
2. Quality comes from machines, not from the user's sign-off: a mechanically computed reviewer floor — one reviewer only when the checker proves the whole change is narrow and low-risk, and two reviewers for every other or undecidable change — plus a blind run by someone who did not write it (omitted only when every Acceptance line is mechanical), and an adversarial pass; every incident becomes a permanent eval. A step the user skips by plain instruction falls outside this guarantee; every such skip must be disclosed on the pull request, and the attestation-level verification status there is recomputed rather than claimed.
