...
> **SSOT for `loom-workflow:handoff` v0.1**
...
> Its sister skill is L3 (in-session, human warm reader): `loom-workflow:recap-state`.
...
The L3 sister is `loom-workflow:recap-state` with its own separate bundle.
...
Implementing `loom-workflow:handoff` v0.1 skill. T1 (schema bundle RED test) and
...
- [ ] [P1] T2: Author `loom-workflow/skills/handoff/SKILL.md`
- [ ] [P2] T3: Tri-language READMEs (EN/JA/zh-TW)
- [ ] [P3] T4: Plugin manifest bump `loom-workflow` 2.10.0 → 2.11.0 + `handoff` keyword
- [ ] [P3] T4: Add `handoff` to `loom-workflow/.claude-plugin/plugin.json` keywords array
...
- `loom-workflow/skills/handoff/` — only files in this directory may be edited;
  sister skills are READ-ONLY structural references per Anthropic skill-independence
- `loom-workflow/.claude-plugin/plugin.json` version until T4 — premature bump
...
2. **Portable — degrades gracefully.** Phrase it so it works whether or not the
   new session has the `loom-workflow:handoff` skill installed, and even in a
...
If you have the `loom-workflow:handoff` skill this is its resume mode; if not,
