...
`session_events`/`target_skill_path`/`target_skill_md_content` as its input),
then `wait_agent` for all, then `close_agent` once done. Codex's own runtime
handles the wait-for-all/consolidate step automatically once the spawn
instruction names multiple agents.
