...
> to surface missed-trigger patterns in shipped `loom-workflow:*` and
...
The skill ships with the `loom-workflow` plugin. In Claude Code:

```
Skill(skill: "loom-workflow:distill-sessions")
```
...
cd loom-workflow/skills/distill-sessions/scripts
python3 main.py
```
