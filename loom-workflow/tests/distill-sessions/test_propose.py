...
_SCRIPTS_DIR = Path(__file__).resolve().parents[2] / "skills" / "distill-sessions" / "scripts"
_FIXTURE_INPUT = _SCRIPTS_DIR / "fixture_subagent_results.json"
_FIXTURE_EXPECTED = Path(__file__).resolve().parent / "fixture_expected_proposals.md"
...
def test_snapshot_matches_expected_output(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            str(out_path),
            "--skill-content-file",
            str(Path(__file__).resolve().parent / "fixture_sample_skill.md"),
        ],
