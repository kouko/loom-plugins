...
- **change** — Editing one capability touches that capability's file and its own test, and nothing else.
- **add** — Adding a capability is a new file plus one routing entry, and every existing capability file stays untouched.
- **remove** — Removing a capability deletes its file and its routing entry, and leaves nothing behind that still names it.
