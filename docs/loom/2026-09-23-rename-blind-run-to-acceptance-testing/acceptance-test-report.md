...
- **我怎麼試**：逐一找出每份說明文件、原則文件、各站流程說明中第一次提到這個步驟的地方；再實際產生一行「PR 上揭露被跳過步驟」的文字，看它怎麼寫。也搜尋了獨立的「UAT」字樣。
- **結果**：三份語言版本的說明文件、根目錄說明、原則文件，第一次提到時都寫「independent acceptance testing」；任何地方都沒有「UAT」。但 PR 上列出被跳過步驟的那一行，印出的是步驟代號 `acceptance-test`，不是「independent acceptance testing」這個說法，流程說明也沒有要求在 PR 上補上完整名稱。
- **證據**：`loom-code/README{,.ja,.zh-TW}.md` 第一次出現於流程圖（第 23／25 行）；`PRINCIPLES.md` 非協商原則第 2 條；`git grep -n -w UAT`（範圍內 0 筆）；以乾淨副本的揭露函式產生：`Skipped steps: spec, acceptance-test — authority: user-typed (AB12, 2026-09-23)`
- **判定**：partly — 文件與說明都做到了；PR 上跳過步驟的那一行只寫代號，沒有寫出完整名稱。
...
- PR 上列出被跳過步驟時只寫 `acceptance-test` 這個代號，你是否希望旁邊也寫出「independent acceptance testing」？
...
- 內部用語清單把「independent acceptance testing」列為對你說話時要換成白話的詞，但第 3 條又要求它作為對你顯示的正式名稱。兩者要怎麼並存？
