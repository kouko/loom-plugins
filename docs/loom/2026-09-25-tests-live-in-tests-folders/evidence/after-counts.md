...
|---|---|---|
| code | `tests loom-code/tests` (xdist) | 2319 passed, 2 skipped |
...
Totals: pytest 3621 passed, 6 skipped (code 2319/2, design 247/1,
...
|---|---|---|---|
| code | 2271 passed, 2 skipped | 2319 passed, 2 skipped | +48 passed |
...
| pytest total | 3568 passed, 6 skipped | 3621 passed, 6 skipped | +53 passed |
...
the folder, not the key). No baseline test is missing after the move; 53 tests
...
| code | `tests/test_tests_folder_convention.py` | +11 | W1-03 repo-wide guard: no tracked or untracked test file outside a `tests/` folder or `docs/loom/` (1), five planted strays refused (root `scripts/`, `.claude/hooks/`, a plugin `hooks/`, a new plugin's `contract/` and `scripts/`), five planted tests allowed (root `tests/`, `tests/hooks/`, a new plugin's `tests/`, a skill tests folder, `docs/loom/` evidence) |
...
Reconciliation: code 23 probes + 5 + 2 + 1 + 6 + 11 = 48; design 2; workflow 1 + 2 = 3;
48 + 2 + 3 = 53 = 3621 − 3568. The 24 tests the baseline recorded as never
