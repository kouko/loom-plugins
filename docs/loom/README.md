...
| `<change-id>/` | The change folder: `spec.md` (only when the change needs design), `plan.md` (the Task DAG), `acceptance-test-report.md` (when independent acceptance testing ran), `attestation.json` (generated by `finalize-review`, bound to the functional-content digest), `evidence/` (everything a reviewer, acceptance tester or end-of-Build adversary produced for THIS change) | write-spec, write-plan, build, closing-review |
...
the end of Build, then read and acceptance testing in review.
...
stores have since been removed and survive only in git history. The old plans stay in
...
acceptance tester＝`acceptance-tester`、讀者＝`reviewer`、對抗者＝`adversary`
（另有實作者＝`implementer`）。圖中縮寫：impl／adv／tester／rev x2。
對抗者在 end of Build 出場，acceptance tester 與讀者在收尾審查出場。
步驟整體先後排列；僅 5 的兩位讀者彼此並行。

```
┌───────┐   ┌──────┐   ┌─────┐   ┌────────┐      ┌────────┐   ┌────────┐   ┌──────┐
│ build │   │ impl │   │ adv │   │ review │      │ tester │   │ rev x2 │   │ ship │
└───┬───┘   └───┬──┘   └──┬──┘   └────┬───┘      └────┬───┘   └────┬───┘   └───┬──┘
    │           │         │           │               │            │           │   
    │           │         │           │               │            │           │   
    │  1 實作   │         │           │               │            │           │   
...
| 4 | review→tester | 逐條走 Acceptance、commit 報告 | 讀者讀之前落地 |
| 5 | review→rev x2 | 兩位讀者判讀 | 等 acceptance test report 落地才派、互不可見 |
