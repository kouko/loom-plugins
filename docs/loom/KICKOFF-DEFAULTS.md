...
- second-vendor: suggest — report cross-vendor review availability without blocking; recommend it for grounded high risk and require opt-in before Closing Review (2026-09-15)
- package-tests: uv run --isolated --with-requirements requirements-package-tests.lock python scripts/run_package_tests.py --loom-family -q — the runner is the single inventory for every Loom pytest and shell-test surface; CI selects named groups from the same inventory, while the complete suite runs at the end of Build and again in finalize-review; isolated invocations preserve per-directory pytest configurations and module names; the hash-verified lock avoids host-Python drift; the code group retains pytest-xdist `-n auto`; a bare pytest at the repo root aborts on dbt-wiki collection (2026-09-15)
