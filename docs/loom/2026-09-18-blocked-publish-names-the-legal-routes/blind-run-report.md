...
**第二次盲跑最重要的一件事，必須先講**：我在團隊要我複跑的那個版本 `057d0c43` 上，**跑出了一個真實的測試失敗**——整包測試結束碼是 1。原因是一個被標成「預期會失敗」的對抗測試已經被修好、卻沒有把標記拿掉，測試框架因此把「意外通過」判為失敗。這個問題在我複跑期間由後續兩個 commit（`3635c03f`、`fc5b8401`）解掉了，我在最新的 `fc5b8401` 上重跑，整包測試結束碼 0。細節在第 3 節第 8 條與第 5 節。
...
|---|---|---|
| 拒絕訊息的結尾 | 只有一種：講兩條路 | **四種**，依分支狀態擇一（見第 3 節第 1 條） |
...
| 已知限制 | 四項（L1–L4） | **四項，但內容換過**：舊的 L1 已經被修掉，另外新增一項（見第 5 節） |
| spec 的建置前審查欄位 | `not-required` | **`required — owed and not run`**（見第 4 節） |
...
團隊要我對照目前的對抗測試模組重新清點。結果是**目前仍有四項**，但**內容和第一次盲跑那四項不一樣**：第一次的 L1 已經被修掉，另外新增一項。

每一項都寫成三段：**你會看到什麼**、**這代表什麼**、**該怎麼做**。

### 已經解掉的：舊 L1 — 內容已經進主線的分支，會卡在審查站原地打轉

第一次盲跑時，這是一個真實的無窮迴圈：拒絕訊息叫你去跑審查站，但那個分支跑幾次都不會脫離。現在拒絕訊息對這個狀態有專屬的一句話——「這個分支沒有東西可發佈，你要發的變更應該從一份新的 intent 重來」——不再指名一條走不通的路。我實跑確認了（第 4 節第 1 條第二列），對應的對抗測試也從「預期失敗」變成通過。**這一項不再是限制。**

### L1（新編號）— 攔截程式看的是它當下讀到的 PR 內文，不是 PR 最後真正收到的內文

- **你會看到什麼**：**看不到任何徵兆。** 這一點按其構造就是不可見的。
- **這代表什麼**：攔截程式在檢查開 PR 指令的那一刻讀一次 PR 內文檔，`gh` 之後會再讀一次。任何在這兩次讀取之間改寫這個檔案的行為，攔截程式都看不到。
- **該怎麼做**：把攔截程式的放行理解成「這份內文在被檢查的那一刻是誠實的」這項證據，不要理解成對最終 PR 內容的保證。ship 站說明的第 3 節已經寫明這件事。對應的測試：`test_probe_body_can_change_between_the_check_and_the_request`。
...
### L4（本次新增）— 檢查站的說明文字有沒有亂開支票，只看破折號前面那半句

- **你會看到什麼**：**看不到任何徵兆。** 這是一個檢查本身的盲點，不是流程跑起來會出現的症狀。
- **這代表什麼**：有一項測試負責確保 ship 站的那句規則不會承諾一條檢查程式不一定會指名的路。這項測試只讀破折號（`—`）前面那半句，所以一句寫在破折號後面的無條件承諾，它看不見。
- **該怎麼做**：日後有人改動 ship 站那句規則時，不要以為那項測試會攔下錯誤的承諾；破折號後面的字要人自己讀過。這一項被明文記成「已測量的限制」而不是缺陷，因為那句規則只是指引、真正能擋的是拒絕訊息本身。對應的測試：`test_probe_scoping_dash_hides_a_promise_placed_after_it`。
