...
change, and no test assertion changes except the one narrowed comparison
below.
...
- Narrowed comparison: in the coldread branch-end graduated probe,
  ...
  lets only a `graduated_dir =` assignment line differ between a graduated
  copy and its evidence original, and skips the one pair where the file
  compares itself with its own original, because the move had to edit that
  file. Any other differing line still fails.
