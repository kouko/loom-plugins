...
- Add `loom_checker.py sync-trunk` (rule `review.sync`): it fetches the remote
  ...
  branch already contains the tip, aborts a conflicting merge and names every
  conflicting file, refuses the trunk, a detached HEAD or a dirty worktree,
  and warns and continues when the remote cannot be reached.
