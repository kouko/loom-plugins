...
**1. Hard cap: 2 review rounds.** After round 2 ends with NEEDS_REVISION (PASS and PASS_WITH_NOTES are both passing verdicts and end the review) → STOP and surface the surviving findings to the user. Hand them the decision. A third round runs ONLY on explicit user authorization (the critics' user-authorized breach precedent) — never silently. **Why a cap rather than "review until clean": a clean round is not a reachable state.** Each pass samples a large pool of genuine small defects, so "the reviewer found nothing" cannot serve as the termination condition — measured on one branch's twelve already-passed `.md` artifacts — four fresh arms, seven gating findings, zero overlap, each traced to its cited text and two confirmed by running the cited command; the audit's §Limits states it does not generalize a rate (`docs/loom/audits/2026-08-04-docs-review-convergence-experiment.md`). The corollary is operative: when a finding names a class a mechanism could hold, **prefer adding the deterministic check over authorizing another round** — a checker returns the same finding every run, a reviewer a different subset each time.
