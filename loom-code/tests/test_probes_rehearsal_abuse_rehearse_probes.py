...
def test_rehearseProbes_helpFlag_exitsZeroNamingEveryArgument(tmp_path: Path) -> None:
    ...
    text = out(proc)
    assert proc.returncode == 0, text
    for token in ("--repo", "--keep", "loom-code/tests/test_probes_*.py"):
...
def test_rehearseProbes_noPositionalPaths_expandsOnlyTheProbeGlob(tmp_path: Path) -> None:
    """The default is `loom-code/tests/test_probes_*.py` and nothing wider:
    a failing neighbour one character outside the glob must not be collected."""
    repo = make_repo(tmp_path, trunk="main")
    commit_file(repo, "loom-code/tests/test_probes_green.py", GREEN_PROBE, "matching probe")
    commit_file(repo, "loom-code/tests/test_probe_red.py", RED_PROBE, "near miss")
    commit_file(repo, "loom-code/tests/test_other_red.py", RED_PROBE, "unrelated test")
