...
def wc_words(data: bytes) -> int:
    """Word count = Python str.split() on the decoded output -- the only
    counter that agrees on macOS and GNU (BSD wc and GNU wc disagree in
    every locale: 3 words apart under UTF-8, 147 apart under C where GNU
    drops all-non-ASCII words). KICKOFF-DEFAULTS records this method."""
...
def run_measure(repo: Path) -> int:
    ...
    # the loom-design architecture tool is explicitly admitted in release 3.16.0.
    if skill_count > 23:
        print(f"RED: skill count {skill_count} exceeds the loom budget of 23")
    if artifact_types > 8:
        print(f"RED: artifact-type count {artifact_types} exceeds the per-change budget of 8")
    print(f"session-start word count: {words}")

    exit_code = 1 if (skill_count > 23 or artifact_types > 8) else 0
    if words < 0:
